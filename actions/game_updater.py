...
def _click_first_profile_by_screenshot() -> bool:
    ...
    try:
        ...
        if not colorful.any():
            print("[GameUpdater] ⚠️ Avatar color not found — estimating click position")
...
def _handle_install_dialog(game_name: str) -> str:
    ...
    try:
        ...
        for _ in range(40):
            ...
            try:
                for hwnd in findwindows.find_windows(
                    title_re=r"(?i)(install|setup|steam)", visible_only=True
                ):
                    try:
        ...
        install_clicked = _click_button(
            dialog, ["install", "setup", "next", "next", "ok", "ok"]
        )
...
def _schedule_mac(hour: int, minute: int) -> str:
    ...
    plist_path  = plist_dir / "com.mj.gameupdater.plist"
    script_path = Path(__file__).resolve()
    plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
    <key>Label</key><string>com.mj.gameupdater</string>
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Delete", "/TN", "MJ_GameUpdater", "/F"],
            capture_output=True, text=True, **_CNW
        )
        return ("Scheduled update cancelled."
                if result.returncode == 0 else "No scheduled update found.")
    if is_mac():
        plist_path = Path.home() / "Library" / "LaunchAgents" / "com.mj.gameupdater.plist"
...
def _get_schedule_status() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Query", "/TN", "MJ_GameUpdater", "/FO", "LIST"],
            capture_output=True, text=True, **_CNW
        )
        if result.returncode != 0:
