...
def _is_safe_path(target: Path) -> bool:
    """Check if given path is inside _SAFE_ROOTS. If not, reject operation."""
    try:
...
def _resolve_path(raw: str) -> Path:
    ...
    lower = raw.lower()
    for key, base_path in shortcuts.items():
        if lower == key:
            return base_path
...
def delete_file(path: str, name: str = "") -> str:
    try:
        ...
        # Safe directory check — protect critical user directories
        protected = {
...
def organize_desktop() -> str:
    ...
    try:
        for item in desktop.iterdir():
            # Do not modify directories, hidden files, or organized folders
