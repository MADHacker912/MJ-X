...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".mj_profiles" / browser
...
def _open_native(url: str, browser_name: Optional[str]) -> str:
    """
    Opens user REAL browser with their profile,
    logged-in accounts, and extensions. No automation is attached,
    so about:blank or empty profile never appears.
    If url is empty, browser launches with default home /
    session restore, exactly as if user launched it directly.
    ...
    if browser_name:
        ...
        # No URL -> opens default browser window
...
class _BrowserSession:
    ...
    All browsers are launched using launch_persistent_context with user profile.
    """
    ...
    async def _adopt_page(self) -> Page:
        ...
        Takes over that initial tab instead of opening a redundant about:blank —
        so the user does not see extra blank tabs.
        """
    ...
    async def _launch(self):
        """
        Launches browser with real user profile.
        If context is already open, does nothing.
        """
        ...
        if engine_name == "firefox":
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using MJ profile")
                mj = str(Path.home() / ".mj_profiles" / "firefox_mj")
                Path(mj).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(mj, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".mj_profiles" / "safari")
        ...
        # Real user profile could not be attached (browser already running / locked profile).
        # Falling back to persistent MJ automation profile — logged in sessions remain persistent.
        mj_profile = str(Path.home() / ".mj_profiles" / self.browser_name)
        Path(mj_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with MJ profile: {mj_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(mj_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with MJ profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "mj_screenshot.png")
...
class _SessionRegistry:
    ...
    def pop_native_url(self) -> str:
        """Returns the last natively opened URL once (consumed to prevent repetition)."""
            ...
            if r.status_code == 200:
                for item in r.json():
                    if item.get("type") == "page":
                        t_title = item.get("title", "Tab")
                        t_url = item.get("url", "")
                        tabs.append(f"🌐 {t_title} ({t_url})")
        except Exception:
            pass
        ...
        try:
            ...
            if res.returncode == 0:
                for l in res.stdout.splitlines():
                    if any(b in l.lower() for b in ['chrome', 'chromium', 'brave', 'edge', 'firefox']):
                        parts = l.split(None, 3)
                        if len(parts) >= 4:
                            title = parts[3].strip()
                            if title and not any(title in t for t in tabs):
                                tabs.append(f"📑 {title}")
        except Exception:
            pass

    if not tabs:
        return "Could not detect open browser tabs or browser is closed."
    return f"Detected Open Browser Tabs ({len(tabs)} total):\n" + "\n".join(tabs)
...
def browser_control(
    ...
    if action in ("list_tabs", "count_tabs", "tabs", "get_tabs"):
    ...
    # ── Navigation ALWAYS native ─────────────────────────────────────────────
    # go_to / search / new_tab opens site in user native browser with user profile.
    # Controlled automation window never appears unless there is an ongoing multi-step automation workflow.
    ...
    # ── Interactive Automation Actions (click/type/read) ────────────────────────
    # Requires a controlled browser instance;
    # opens automation window and navigates to target page
    # directly without waiting on blank tab.
    try:
        sess = _registry.get(browser)
    except Exception as e:
