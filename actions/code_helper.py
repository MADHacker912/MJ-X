...
def _clean_code(text: str) -> str:
    text = text.strip()
    # Extract block code if model added explanation before/after code blocks
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"mj_debug_{int(time.time())}.png"
...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    """
    Language-independent intent detection via LLM.
    Classifies user intent regardless of input language (English, Hindi, etc.).
    Falls back to structural code cues if API is unreachable.
    ...
    # Structural fallback — language agnostic
    if file_exists:
