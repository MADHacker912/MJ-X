...
_EDGE_TTS_VOICE_BY_GENDER = {
    "male": "en-US-GuyNeural",
    "female": "en-US-JennyNeural",
}

_KOKORO_VOICE_BY_GENDER = {
    "male": "am_heart",
    "female": "af_heart",
}
...
def _import_kokoro_pipeline():
    ...
    if sys.version_info >= (3, 13):
        raise RuntimeError(
            "Kokoro TTS is unavailable on Python 3.13 or newer. "
            "Use Edge TTS, or run MJ with Python 3.12."
        )
...
def create_tts_player(config: dict) -> TTSPlayer:
    ...
    gender = str(config.get("tts_voice_gender", "female")).strip().lower()
    if engine_name == "kokoro":
        voice = config.get(
            "tts_voice",
            _KOKORO_VOICE_BY_GENDER.get(gender, _KOKORO_VOICE_BY_GENDER["female"]),
        )
        ...
        voice = config.get(
            "tts_voice",
            _EDGE_TTS_VOICE_BY_GENDER.get(gender, _EDGE_TTS_VOICE_BY_GENDER["female"]),
        )
