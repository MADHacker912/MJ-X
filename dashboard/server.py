"""
dashboard/server.py — MJ Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "MJ Uploads",
        Path.home() / "Documents" / "MJ Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"MJ Dashboard Port {port}"
        prog_rule  = "MJ Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="mj_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until MJ is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "mj.key").exists() and (certs / "mj.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in MJ to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('mj_token','{tok}');
  sessionStorage.setItem('mj_key','{key}');
  localStorage.setItem('mj_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to MJ…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "mj.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "mj.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in MJ UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
